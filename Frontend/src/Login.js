...
export default function Login({ onBack, onLoginSuccess, startTemporaryListening, generateAudio, isListening, isSpeaking }) {
  ...
  const startListeningForField = (field) => {
    if (isListening || isSpeaking) {
      console.log('Login: Blocking startListeningForField because App is busy (isListening:', isListening, ', isSpeaking:', isSpeaking, ')');
    ...
    console.log('Login: Directly attempting to start field listening for', field);

    startTemporaryListening( // Use the generic listener
      (text) => { // onResult callback
        console.log('Login: Speech recognized for', field, ':', text);
      ...
      (error) => { // onError callback
        console.error('Login: Error in field recognition for', field, ':', error);
        setMessage('Error: ' + error);
        setCurrentFieldListening(null);
        generateAudio('பேச்சை அடையாளம் காண முடியவில்லை அல்லது ஒரு பிழை ஏற்பட்டது. மீண்டும் முயற்சிக்கவும்.');
      },
      () => { // onNoInput callback
        console.log('Login: No input detected for field:', field);
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    setMessage('உள்நுழையப்படுகிறது...');
    generateAudio('உள்நுழையப்படுகிறது.');
    try {
      ...
      if (!res.ok) {
        const errorMessage = data.msg || 'உள்நுழைவு தோல்வியடைந்தது';
        setMessage(`Error: ${errorMessage}`);
        generateAudio(`தவறு: ${errorMessage}`);
      } else {
        setMessage('Success: உள்நுழைவு வெற்றிகரமானது!');
        generateAudio('உள்நுழைவு வெற்றிகரமானது!');
        if (onLoginSuccess) {
  ...
  const getMessageClass = () => {
    ...
    if (message.includes('உள்நுழையப்படுகிறது')) return 'info';
    return '';
  };
